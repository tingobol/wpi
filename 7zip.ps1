...
Try {
    ...
    [string]$installName = ''
    [string]$installTitle = 'Igor Pavlov 7-Zip'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        ## Install 7-Zip (32-bit)
        $ExePath32 = Get-ChildItem -Path "$dirFiles\x86" -Include "7z*.exe" -File -Recurse -ErrorAction SilentlyContinue
        $MsiPath32 = Get-ChildItem -Path "$dirFiles\x86" -Include "7z*.msi" -File -Recurse -ErrorAction SilentlyContinue
        $Transform32 = Get-ChildItem -Path "$dirFiles\x86" -Include "*.mst" -File -Recurse -ErrorAction SilentlyContinue

        If($ExePath32.Exists)
        {
        Write-Log -Message "Found $($ExePath32.FullName), now attempting to install $appName."
        Show-InstallationProgress "Installing 7-Zip (32-bit). This may take some time. Please wait..."
        Execute-Process -Path "$ExePath32" -Parameters "/S" -WindowStyle Hidden
        Start-Sleep -Seconds 5
        }

        ElseIf(($MsiPath32.Exists) -and ($Transform32.Exists))
        {
        Write-Log -Message "Found $($MsiPath32.FullName) and $($Transform32.FullName), now attempting to install $appName."
        Show-InstallationProgress "Installing 7-Zip (32-bit). This may take some time. Please wait..."
        ...
        If($FileAssoc){
        Write-Log -Message "Configuring $appName File Associations (32-bit)"
    ...
    ElseIf ($deploymentType -ieq 'Uninstall')
    {
        ...
        ## Uninstal Any Existing Versions of 7-Zip (EXE)
        $AppList = Get-InstalledApplication -Name '7-Zip'        
        ForEach ($App in $AppList)
        {
        If($App.UninstallString)
